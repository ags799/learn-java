...
public class DynamoDbPathTable implements PathTable {
  private static DynamoDbPathTable instance = null;

  private final Dao<DynamoDbPathEdge> dao;

  @VisibleForTesting
  DynamoDbPathTable(Dao<DynamoDbPathEdge> dao) {
    this.dao = dao;
  }

  public static DynamoDbPathTable getInstance() {
    if (instance == null) {
      Dao<DynamoDbPathEdge> dao =
          DynamoDbDao.create(DynamoDbPathEdge.class, Environment.getPathTableName());
      instance = new DynamoDbPathTable(dao);
    }
    return instance;
  }
